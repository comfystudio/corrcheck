...
function create_corrCheck(){
  ...
  <form class="corrCheck_form form-horizontal" role="form" method="post" action="output.php"> 
  ...
  <button id="submit" name="submit" type="submit" class="btn btn-success">Submit Report</button>

  <button id="save" name="save" type="submit" class="btn btn-warning">Submit Report</button>
   ...
   <fieldset class="section_rep_details rep-section">
    ...
    <?php 
      ...
      while ($row = $results->fetch(PDO::FETCH_ASSOC)):
        ...
        ?>
          ...
          <div class="form-group cf">
            ...
            <?php if($question_type == "Textarea"){ ?>          
              <textarea name="<?php echo $question_name ?>" id="<?php echo $question_name ?>" rows="6" class="form-control"></textarea>
            <?php } 
              elseif($question_id==169) { // If question is: inspection completed by ?>  

              <input type="text" class="form-control form-control" maxlength="50" name="<?php echo $question_name ?>" id="<?php echo $question_name ?>" readonly value="<?php echo $_SESSION["user"]["first_name"] . " " . $_SESSION["user"]["last_name"]; ?>">  
              <input type="hidden" name="current_username_id" value="<?php echo $_SESSION["user"]["user_id"] ?>">

                <p>If editing an EXISTING report this will show username of the person who completed the report</p>

            <?php } elseif($question_id==170) { // If question is: inspection supervised by ?>  

               <select name="veh_rep_dets_supervised_by_<?php echo $question_id; ?>"
              id = "veh_rep_dets_supervised_by_<?php echo $question_id; ?>" class="form-control">
        ...
        <div class="alert alert-info" role="alert">
          <strong>NOTE:</strong> When survey is marked as "FINAL" by a manager-level user an option will appear asking if the report is to be emailed to the customer.
      </div>
